...
| Route | Output |
|---|---|
| `GET /` | HTML index of all sites + recent items + feed links |
...
| `feed_item_limit` | 10 | `serve --limit` / `--all` |
...
| Export | Signature | Notes |
|---|---|---|
| `createApp` | `(db, config, opts?: { feedLimit?: number }) → Hono` | Single catch-all route; see [HTTP routes](#3-http-routes). `feedLimit 0` = every stored article |
